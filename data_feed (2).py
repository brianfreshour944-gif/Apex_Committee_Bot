...
async def get_account_state() -> tuple[float, float]:
    ...
    FIXED: was a plain `def`, but main.py calls it with
    `await get_account_state()` -- the exact same bug class as
    get_orderbook_ratio, confirmed by direct testing (TypeError: object
    tuple can't be used in 'await' expression). This fires on essentially
    every single main-loop cycle, right after write_heartbeat() -- meaning
    the bot could never get past fetching account state at all.
    """
...
async def get_all_positions() -> dict:
    """
    FIXED: same bug -- was a plain `def`, main.py calls it with
